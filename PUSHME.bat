...
echo wscript.Quit((msgbox("Aprire DNSOptimizer?",4+32, "")-6) Mod 255) > %temp%\DNSmsg.vbs
...
start /w powershell.exe -ExecutionPolicy Bypass -File ".\reg.ps1"
timeout 5
:Internet
start /w %temp%\Internetmsg.vbs
if errorlevel 1 goto Clean
start /w .\Internet.bat
start /w %temp%\DNSmsg.vbs
if errorlevel 1 goto Clean
start /w powershell.exe -ExecutionPolicy Bypass -File ".\src\dns.ps1"
timeout 5
:Clean
