...
# Recupera TUTTE le app installate in un colpo solo (molto più veloce)
$lblLoad.Text = "Recupero lista app installate..."
[System.Windows.Forms.Application]::DoEvents()

$wingetListRaw = & winget list --accept-source-agreements 2>&1 | Out-String
$wingetInstalled = @{}

$allAppsMapping = @{}
foreach ($k in $microsoftApps.Keys)  { $allAppsMapping[$k] = $microsoftApps[$k] }
foreach ($k in $thirdPartyApps.Keys) { $allAppsMapping[$k] = $thirdPartyApps[$k] }

$allKeys = @($microsoftApps.Keys) + @($thirdPartyApps.Keys)
foreach ($key in $allKeys) {
    $appName = $allAppsMapping[$key]
    
    # Ricerca intelligente: controlla se l'ID O il Nome compaiono nella lista winget
    # Questo risolve i problemi con WhatsApp, UniGetUI e Dolby Access
    $idMatch = $wingetListRaw -match [regex]::Escape($key)
    $nameMatch = $false
    if ($appName) {
        $nameMatch = $wingetListRaw -match [regex]::Escape($appName)
    }
    
    $wingetInstalled[$key] = ($idMatch -or $nameMatch)
}
...
if ($Silent -and $ApplyOnly.Count -gt 0) {
    ...
    Write-Host "[*] Installazione silenziosa di $($ApplyOnly.Count) app..." -ForegroundColor Cyan
    foreach ($appId in $ApplyOnly) {
        $appName = if ($allApps.ContainsKey($appId)) { $allApps[$appId] } else { $appId }
        Write-Host "  -> $appName ($appId)..." -ForegroundColor Yellow
        try {
            ...
            switch ($exitCode) {
                0                { Write-Host "    [OK] Installata." -ForegroundColor Green }
                -1978335189      { Write-Host "    [--] Già installata." -ForegroundColor Gray }
                -1978335209      { Write-Host "    [--] Già all'ultima versione." -ForegroundColor Gray }
                -1978335212      { Write-Host "    [!!] Nessun installer compatibile." -ForegroundColor Red }
                -1978335215      { Write-Host "    [!!] Pacchetto non trovato." -ForegroundColor Red }
                default          { Write-Host "    [!!] Errore (codice: $exitCode)." -ForegroundColor Red }
            }
...
$MainProgressBar.Location = New-Object System.Drawing.Point(130, 470)
$MainProgressBar.Size = New-Object System.Drawing.Size(540, 14)
...
$btnUpgrade.Text = "AGGIORNA TUTTO"
$btnUpgrade.Location = New-Object System.Drawing.Point(295, 421)
$btnUpgrade.Size = New-Object System.Drawing.Size(125, 44)
$btnUpgrade.BackColor = [System.Drawing.Color]::DarkOrange
$btnUpgrade.ForeColor = [System.Drawing.Color]::White
$btnUpgrade.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
$btnUpgrade.Add_Click({
    ...
    $LogBox.AppendText("=== Avvio aggiornamento di tutte le app installate... ===`r`n")
    [System.Windows.Forms.Application]::DoEvents()

    try {
        # Avvia winget upgrade in un processo separato e legge l'output riga per riga
        $psi = New-Object System.Diagnostics.ProcessStartInfo
        $psi.FileName = "winget"
        $psi.Arguments = "upgrade --all --include-unknown --silent --disable-interactivity --accept-source-agreements --accept-package-agreements"
        ...
        # Legge stdout riga per riga in modo asincrono aggiornando il log in tempo reale
        ...
        if ($proc.ExitCode -eq 0) {
            $LogBox.AppendText("`r`n[OK] Aggiornamento completato con successo.`r`n")
        } elseif ($proc.ExitCode -eq -1978335209) {
            $LogBox.AppendText("`r`n[--] Nessun aggiornamento disponibile.`r`n")
        } else {
            $LogBox.AppendText("`r`n[!!] Aggiornamento terminato con codice: $($proc.ExitCode).`r`n")
        }
    } catch {
        $LogBox.AppendText("[!!] Errore durante l'aggiornamento: $_`r`n")
    }

    $LogBox.AppendText("=== Fine aggiornamento. ===`r`n")
    $LogBox.ScrollToCaret()
...
$btnInstalla.Add_Click({
    ...
    $MainProgressBar.Maximum = $selectedApps.Count
    $stepCount = 0
    ...
    foreach ($appId in $selectedApps) {
        ...
        # Animazione di caricamento mentre winget lavora
        ...
        try {
            ...
            # Torna alla modalità continua per mostrare l'avanzamento reale
            $MainProgressBar.Style = "Continuous"
            $stepCount++
            $MainProgressBar.Value = $stepCount
...
#idea : aggiungi un tasto  Winget upgrade --all --include-unknown --disable-interactivity --silent --accept-source-agreements --accept-package-agreements